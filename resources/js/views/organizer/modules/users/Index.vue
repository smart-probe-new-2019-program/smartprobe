        ...
        <div class="card">
          <div class="card-header">
            <div class="row">
              ...
              <div class="col-sm-6">
                <router-link to="/organizer/users/add" class="btn btn-success btn-xs float-right">
                  <i class="icon-fa icon-fa-plus-circle" />Add User
                </router-link>
              </div>
            </div>
          </div>
              ...
              >
                ...
                <table-column :sortable="false" :filterable="false" label="Actions">
                  <template slot-scope="row">
                    <a :href="`/organizer/users/view/${row.id}`">
                      <i class="icon-fa icon-fa-eye" />
                    </a>
                    <a :href="`/organizer/users/edit/${row.id}`">
...
export default {
  ...
  methods: {
    getUsers: lodash.debounce(function(page = 1) {
      ...
      axios
        .get(
          `/api/admin/users/get?page=${page}&filter_role=${app.filters.role}&filter_keyword=${app.filters.keyword}&organization_id=${app.organization_id}`
        )
        .then(function(resp) {
          app.users = resp.data;
        })
