        ...
        <div class="card">
          <div class="card-header">
			  	<div class="row">
					...
					<div class="col-sm-6">
						<router-link to="/organizer/corrective_actions/add" class="btn btn-success btn-xs float-right"><i class="icon-fa icon-fa-plus-circle"/>Add Action</router-link>
					</div>
			  	</div>  
          </div>
          <div class="card-body">
				<table-component :data="fetchData" :show-filter="false" filter-placeholder="Search corrective actions.." filter-no-results="No actions found!" table-class="table" ref="table">
					...
					<table-column :sortable="false" :filterable="false" label="Actions">
						<template slot-scope="row">
							<a :href="`/organizer/corrective_actions/view/${row.id}`"><i class="icon-fa icon-fa-eye"/></a>
							<a :href="`/organizer/corrective_actions/edit/${row.id}`"><i class="icon-fa icon-fa-pencil-square-o"/></a>
...
export default {
  ...
  methods: {
    ...
    async fetchData ({ page, filter, sort }) {
      const response = await axios.get(`/api/admin/corrective_actions/get?page=${page}&organization_id=${this.organization_id}`)
