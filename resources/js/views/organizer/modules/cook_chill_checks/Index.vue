        ...
        <div class="card">
          <div class="card-header">
			  	<div class="row">
					...
					<div class="col-sm-6">
						<router-link to="/organizer/cook_chill_checks/add" class="btn btn-success btn-xs float-right"><i class="icon-fa icon-fa-plus-circle"/>Add Cook Chill Check</router-link>
					</div>
			  	</div>  
          </div>
          <div class="card-body">
				<table-component :data="fetchData" :show-filter="false" filter-placeholder="Search cook chill checks.." filter-no-results="No entries found!" table-class="table" ref="table">
					...
					<table-column :sortable="false" :filterable="false" label="Actions">
						<template slot-scope="row">
							<a :href="`/organizer/cook_chill_checks/view/${row.id}`"><i class="icon-fa icon-fa-eye"/></a>
							<a :href="`/organizer/cook_chill_checks/edit/${row.id}`"><i class="icon-fa icon-fa-pencil-square-o"/></a>
...
export default {
  ...
  methods: {
    ...
    async fetchData ({ page, filter, sort }) {
      const response = await axios.get(`/api/admin/cook_chill_checks/get?page=${page}&organization_id=${this.organization_id}`)
