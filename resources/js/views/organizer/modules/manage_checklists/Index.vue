        ...
        <div class="card">
          <div class="card-header">
			  	<div class="row">
					...
					<div class="col-sm-6">
						<router-link to="/organizer/manage_checklists/add" class="btn btn-success btn-xs float-right"><i class="icon-fa icon-fa-plus-circle"/>Add Checklist</router-link>
					</div>
			  	</div>  
          </div>
          <div class="card-body">
				<table-component :data="fetchData" :show-filter="false" filter-placeholder="Search checklists.." filter-no-results="No checklists found!" table-class="table" ref="table">
					...
					<table-column :sortable="false" :filterable="false" label="Actions">
						<template slot-scope="row">
							<a :href="`/organizer/manage_checklists/view/${row.id}`"><i class="icon-fa icon-fa-eye"/></a>
							<a :href="`/organizer/manage_checklists/edit/${row.id}`"><i class="icon-fa icon-fa-pencil-square-o"/></a>
...
export default {
  ...
  methods: {
    ...
    async fetchData ({ page, filter, sort }) {
      const response = await axios.get(`/api/admin/manage_checklists/get?page=${page}&organization_id=${this.organization_id}`)
