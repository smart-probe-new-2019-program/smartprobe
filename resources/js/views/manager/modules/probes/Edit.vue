        ...
        <div class="card">
          <div class="card-header">
            <div class="row">
              ...
              <div class="col-sm-6">
                <router-link to="/manager/probes" class="btn btn-dark btn-xs float-right">
...
export default {
	...
	methods: {
		getProbe() {
			...
			axios.get('/api/admin/probes/getProbe/' + id)
			.then(function(resp) {
		...
		saveForm() {
			...
			app.probeData.updated_by = localStorage.getItem("user.id"); //update user id of the one who edited
			axios.patch('/api/admin/probes/' + app.probeData.id, app.probeData)
			.then(function(resp) {
				if(resp.data.status == 'error'){
					toastr['error']('Something went wrong while updating the probe. Please contact admin about this.', 'Error!');
				}
				else{
					app.$router.push('/manager/probes');
					toastr['success']('Probe details updated!', 'Success!');
				}
			})
			.catch(function() {
