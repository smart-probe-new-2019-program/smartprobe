        ...
        <div class="card">
          <div class="card-header">
			  	<div class="row">
					...
					<div class="col-sm-6">
						<router-link to="/manager/probes/add" class="btn btn-success btn-xs float-right"><i class="icon-fa icon-fa-plus-circle"/>Add Probe</router-link>
					</div>
			  	</div>  
          </div>
          <div class="card-body">
				<table-component :data="fetchData" :show-filter="false" filter-placeholder="Search probes.." filter-no-results="No probes found!" table-class="table" ref="table">
					...
					<table-column :sortable="false" :filterable="false" label="Actions">
						<template slot-scope="row">
							<a :href="`/manager/probes/view/${row.id}`"><i class="icon-fa icon-fa-eye"/></a>
							<a :href="`/manager/probes/edit/${row.id}`"><i class="icon-fa icon-fa-pencil-square-o"/></a>
...
export default {
  ...
  data() {
		return {
			location_id: localStorage.getItem("user.location_id"),
		};
	},
  methods: {
	mounted() {
    ...
    async fetchData ({ page, filter, sort }) {
      const response = await axios.get(`/api/admin/probes/get?page=${page}&location_id=${this.location_id}`)
