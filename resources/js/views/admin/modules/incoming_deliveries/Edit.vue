				...
				<div class="panel-body">
					<div class="row">
						<div class="col-sm-6 form-group">
							...
							<select class="form-control" disabled @change="getNeededDropdowns()" v-model.trim="incomingDeliveryData.organization_id" ref="organization_id" required>
...
export default {
	...
	methods: {
		getIncomingDelivery() {
			...
			axios.get('/api/admin/incoming_deliveries/getIncomingDelivery/' + id)
			.then(function(resp) {
		...
		saveForm() {
			...
			app.incomingDeliveryData.updated_by = localStorage.getItem("user.id"); //update user id of the one who edited
			axios.patch('/api/admin/incoming_deliveries/' + app.incomingDeliveryData.id, app.incomingDeliveryData)
			.then(function(resp) {
				if(resp.data.status == 'error'){
					toastr['error']('Something went wrong while updating the delivery. Please contact admin about this.', 'Error!');
				}
				else{
					app.$router.push('/admin/incoming_deliveries');
					toastr['success']('Delivery details updated!', 'Success!');
				}
			})
			.catch(function() {
