        ...
        <div class="card">
          <div class="card-header">
			  	<div class="row">
					...
					<div class="col-sm-6">
						<router-link to="/admin/file_entries/add" class="btn btn-success btn-xs float-right"><i class="icon-fa icon-fa-plus-circle"/>Add File/s</router-link>
					</div>
			  	</div>  
          </div>
          <div class="card-body">
				<table-component :data="fetchData" :show-filter="false" filter-placeholder="Search files.." filter-no-results="No files found!" table-class="table" ref="table">
					...
					<table-column :sortable="false" :filterable="false" label="Actions">
						<template slot-scope="row">
							...
							<a v-on:click="deleteFileEntry(`${row.id}`)"><i class="icon-fa icon-fa-trash"/></a>
						</template>
					</table-column>
				</table-component>
          </div>
...
export default {
  ...
  methods: {
    ...
    async fetchData ({ page, filter, sort }) {
      const response = await axios.get(`/api/admin/file_entries/get?page=${page}`)
	...
	deleteFileEntry(id){
		...
		notie.confirm({
			...
			submitCallback: function () {
				axios.delete('/api/admin/file_entries/' + id)
				.then((resp) => {
					if(resp.data.status == 'error'){
						toastr['error']('Something went wrong while deleting the file. Please contact admin about this.', 'Error!');
					}
					else{
