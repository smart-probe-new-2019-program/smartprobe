      ...
      <div class="col-sm-12">
        <div class="card">
          <div class="card-header">
            <div class="row">
              <div class="col-sm-6">
                <h5>Edit a Cook Chill Check</h5>
              </div>
              <div class="col-sm-6">
                <router-link to="/admin/cook_chill_checks" class="btn btn-dark btn-xs float-right">
				...
				<div class="panel-body">
					<div class="row">
						<div class="col-sm-6 form-group">
							...
							<select class="form-control" disabled @change="getNeededDropdowns()" v-model.trim="cookChillCheckData.organization_id" ref="organization_id" required>
...
export default {
	...
	methods: {
		getCookChillCheck() {
			...
			axios.get('/api/admin/cook_chill_checks/getCookChillCheck/' + id)
			.then(function(resp) {
				app.cookChillCheckData = resp.data;
				app.getNeededDropdowns();
			})
			.catch(function() {
				console.log("Error fetching cook chill check data");
			});
		},
		getOrganizations() {
			let app = this;
		...
		saveForm() {
			...
			app.cookChillCheckData.updated_by = localStorage.getItem("user.id"); //update user id of the one who edited
			axios.patch('/api/admin/cook_chill_checks/' + app.cookChillCheckData.id, app.cookChillCheckData)
			.then(function(resp) {
				if(resp.data.status == 'error'){
					toastr['error']('Something went wrong while updating the entry. Please contact admin about this.', 'Error!');
				}
				else{
					app.$router.push('/admin/cook_chill_checks');
					toastr['success']('Cook chill check details updated!', 'Success!');
				}
			})
			.catch(function() {
