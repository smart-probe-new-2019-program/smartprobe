        ...
        <div class="card">
          <div class="card-header">
			  	<div class="row">
					...
					<div class="col-sm-6">
						<router-link to="/admin/incoming_deliveries_items/add" class="btn btn-success btn-xs float-right"><i class="icon-fa icon-fa-plus-circle"/>Add Item</router-link>
					</div>
			  	</div>  
          </div>
          <div class="card-body">
				<table-component :data="fetchData" :show-filter="false" filter-placeholder="Search items.." filter-no-results="No items found!" table-class="table" ref="table">
					...
					<table-column :sortable="false" :filterable="false" label="Actions">
						<template slot-scope="row">
							<a :href="`/admin/incoming_deliveries_items/edit/${row.id}`"><i class="icon-fa icon-fa-pencil-square-o"/></a>
...
export default {
  ...
  methods: {
    ...
    async fetchData ({ page, filter, sort }) {
      const response = await axios.get(`/api/admin/incoming_deliveries_items/get?page=${page}`)
