        ...
        <div class="card">
          <div class="card-header">
			  	<div class="row">
					...
					<div class="col-sm-6">
						<router-link to="/admin/users/add" class="btn btn-success btn-xs float-right"><i class="icon-fa icon-fa-plus-circle"/>Add User</router-link>
					</div>
			  	</div>  
          </div>
          <div class="card-body">
				<table-component :data="fetchData" :show-filter="false" filter-placeholder="Search users.." filter-no-results="No users found!" table-class="table" ref="table">
					...
					<table-column :sortable="false" :filterable="false" label="Actions">
						<template slot-scope="row">
							<a :href="`/admin/users/view/${row.id}`"><i class="icon-fa icon-fa-eye"/></a>
							<a :href="`/admin/users/edit/${row.id}`"><i class="icon-fa icon-fa-pencil-square-o"/></a>
